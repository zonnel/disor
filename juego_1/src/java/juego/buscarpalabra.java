...
public class buscarpalabra extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String palabra;
        try{
           palabra = request.getParameter("pala");
        }catch(Exception e){
            palabra = "";
        }
        ...
        if(palabra.equals("")){
            palabras = jpalabra.findPalabraEntities();
            
        }else{
            palabras = jpalabra.findPalabra2(palabra);
        }
        
        HttpSession sesionActual = request.getSession(true);
        sesionActual.setAttribute("palabras",palabras);
        sesionActual.setAttribute("pal", palabra);
        ...
        /*try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet buscarpalabra</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet buscarpalabra at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }*/
